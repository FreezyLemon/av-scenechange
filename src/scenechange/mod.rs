...
use v_frame::{frame::Frame, plane::Plane, pixel::Pixel};
...
impl<T: Pixel> SceneChangeDetector<T> {
  pub fn new(
    ...
    lookahead_distance: usize, max_frame_width: u32, max_frame_height: u32,
    min_kf_interval: u64, max_kf_interval: u64
  ) -> Self {
    // Downscaling function for fast scene detection
  ...
  fn initialize_score_deque(
    &mut self, frame_set: &[&Arc<Frame<T>>],
    init_len: usize,
  ) {
    for x in 0..init_len {
      self.run_comparison(
