    ...
    impl<'a, T: Pixel> $name<'a, T> {
      ...
      pub fn subregion(&self, area: Area) -> PlaneRegion<'_, T> {
        ...
        let rect = area.to_rect(
          usize::from(self.plane_cfg.subsampling_x()) >> 1,
          usize::from(self.plane_cfg.subsampling_y()) >> 1,
          self.rect.width,
          self.rect.height,
        );
...
impl<T: Pixel> AsRegion<T> for Plane<T> {
    ...
    fn region(&self, area: Area) -> PlaneRegion<'_, T> {
        ...
        let rect = area.to_rect(
            usize::from(geometry.subsampling_x()) >> 1,
            usize::from(geometry.subsampling_y()) >> 1,
